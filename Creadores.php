...
if (!empty($searchTerm)) {
    ...
    $sql = "SELECT usuario, nombre, correo, foto_perfil, busquedas FROM usuarios";
}
...
if ($searchField === 'usuario' && !empty($searchTerm)) {
   
    foreach ($usuarios as $usuario) {

        if (strcasecmp($usuario['usuario'], $searchTerm) === 0) {

            $usuarioEsc = mysqli_real_escape_string($conexion, $usuario['usuario']);
            $updateSql = "UPDATE usuarios SET busquedas = busquedas + 1 WHERE usuario = '$usuarioEsc'";
